...
Colors are everywhere in every single object. Being able to handle colored objects to control your browser through the camera is very appealing. For that reason, \textit{tracking.js} implemented a basic color tracking algorithm that resulted in a real-time frame rate through a simple and intuitive API.
...
Once all pixels of the tracked color are collected two steps are still required: (1) Detect outliers; and (2) Find the coordinates that represent the color blob.
...
Outlier pixels are determined if they are not close enough to other pixels with the same characteristics. For each pixel $p=(x,y)$, the mean distance with the other found pixels is calculated as follows:
