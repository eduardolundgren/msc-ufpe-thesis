...
\begin{enumerate}
  ...
  \item Optical flow based: rely on spatial information obtained by image-model matching extracted from the relative movement of the object projection onto the image \cite{Teichrieb2007,Lepetit2005}. After initialization, which is often manual, the optical flow between the frames captured at time t and $t+1$ is calculated \cite{Teichrieb2007}. Then, the algorithm determines which points from the model projected onto the image at time t are still present in the image at time $t+1$ \cite{Teichrieb2007}.
