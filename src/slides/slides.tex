...
\begin{frame}{The beggining of the web}
  \begin{figure}[!htb]
    \centering
    \includegraphics[width=180pt]{mosaic_browser.png}
  \end{figure}
\end{frame}
\begin{frame}{The beggining of the web}
  \begin{itemize}
...
\begin{frame}{Browser architecture}
  ...
  \end{figure}
\end{frame}
\begin{frame}{Video capturing and processing workflow}
  \begin{figure}[!htb]
    \centering
...
\begin{frame}{Audio and video}
  ...
  \end{figure}
\end{frame}
\begin{frame}{Capturing audio and video}
  \begin{figure}[!htb]
    \centering
    \includegraphics[width=100pt]{webrtc.png}
  \end{figure}
\end{frame}
\begin{frame}[fragile]{Capturing audio and video}
  \begin{lstlisting}[
                    language=C++,
                    label={lst:get_user_media},
                    caption=Capturing browser microphone and camera,
  ...
  <script>
    ...
    navigator.@2getUserMedia@({video: true, audio: true}, function(localMediaStream) {
...
\begin{frame}{Video capturing and processing workflow}
  \begin{figure}[!htb]
    ...
    \caption{Access flow of raw binary data captured from videos}
...
\begin{frame}{tracking.js}
  \begin{figure}[!htb]
    \centering
    \includegraphics[width=210pt]{site.png}
  \end{figure}
\end{frame}
\begin{frame}{tracking.js}
...
\begin{frame}{Flash vs HTML5}
  \begin{figure}[!htb]
    ...
    \caption{Google trends results for ``flash games'' (top) and for ``html5 games'' (bottom)}
    \label{figure:trend}
  \end{figure}
\end{frame}
\begin{frame}{Library features}
  \begin{figure}[!htb]
    \centering
    \includegraphics[width=190pt]{preview.png}
  \end{figure}
\end{frame}
...
\begin{frame}{Feature detector (FAST)}
  \begin{itemize}
    \item Detects individual features across images
...
\end{frame}
\begin{frame}{Feature extractor (BRIEF)}
  On BRIEF to generate the binary strings it is defined the test $\tau$ on patch \textbf{p} of size \textbf{S $\times$ S} as:
